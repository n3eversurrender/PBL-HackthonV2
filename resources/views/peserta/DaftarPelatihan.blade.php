        ...
        <table id="kursus-table" class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td class="px-6 py-4 text-center">{{ $kursus->firstItem() + $loop->iteration - 1 }}</td>
                    <td class="px-6 py-4">{{ $kursusItem->judul }}</td>
                    <td class="px-6 py-4">{{ $kursusItem->pengguna->nama ?? 'Tidak ada pelatih' }}</td>
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="h-2.5 w-2.5 rounded-full bg-green-500 mr-2"></div>
                            @if($kursusItem->pendaftaran->isNotEmpty())
                            {{ $kursusItem->pendaftaran->first()->status_pendaftaran }}
                            @else
                            Belum terdaftar
                            @endif
                        </div>
                    ...
                    <td class="px-6 py-4">
                        <!-- Button Detail -->
                        <button type="button" data-modal-target="my_modal_detail_{{ $kursusItem->kursus_id }}" data-modal-toggle="my_modal_detail_{{ $kursusItem->kursus_id }}" class="font-medium text-blue-600 dark:text-blue-500 hover:text-blue-700 dark:hover:text-blue-400 mr-4">
                ...
                <!-- Modal Detail -->
                <div id="my_modal_detail_{{ $kursusItem->kursus_id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-2xl max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                ...
                                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="my_modal_detail_{{ $kursusItem->kursus_id }}">
                            ...
                            <div class="p-4 md:p-5 bg-white dark:bg-gray-800 rounded-lg shadow-lg space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->judul }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Deskripsi</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->deskripsi }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Harga</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->harga }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Tingkat Kesulitan</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->tingkat_kesulitan }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Rating</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->rating }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Status</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->status }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Tanggal Mulai</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->tgl_mulai }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Tanggal Selesai</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->tgl_selesai }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Kapasitas</p>
                                        <p class="text-base text-gray-800 dark:text-gray-200">{{ $kursusItem->kapasitas }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <div class="p-4 md:p-5 text-center">
                                ...
                                <form action="{{ route('DaftarPelatihan.destroy', $kursusItem->kursus_id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
